...
var { emojify } = __require("node-emoji");
var log = console.log;
function info(info2) {
  const emoji = emojify(":sparkles:");
  log(emoji, " ", chalk.bold.blue(info2));
}
function error(info2) {
  const emoji = emojify(":exclamation:");
  log(emoji, " ", chalk.bold.red(info2));
}
function warn(info2) {
  const emoji = emojify(":warning:");
  log(emoji, " ", chalk.bold.yellow(info2));
}
function success(info2) {
  const emoji = emojify(":white_check_mark:");
  log(emoji, " ", chalk.bold.green(info2));
}
...
var FileStructures = class {
  ...
  getFileStructures() {
    ...
    if (this.languageType === "javaScript" /* javaScript */) {
      if (!fs.existsSync(this.baseDir + "/nemoCore.js")) {
      ...
      if (!fs.existsSync(this.baseDir + "/nemoCore.ts")) {
        result["nemoCore.ts"] = getTemplatesDirFile("nemoCore.ts", "nemoCore/");
      }
      if (this.templateType && this.checkHasTemplateFetch() && !fs.existsSync(this.baseDir + `/${this.templateType}.ts`) && !fs.existsSync(this.baseDir + `/${this.templateType}.tsx`)) {
        try {
...
var FilesManager = class {
  ...
  diffFiles(newFiles, lastFiles, dir = this.baseDir) {
    ...
    _2.map(lastFiles, (lastValue, name) => {
  ...
  async updateFiles(files) {
    await Promise.all(
      _2.map(files, async (value, filePath) => {
        if (value === void 0) {
  ...
  async generateFiles(files, dir = this.baseDir) {
    ...
    const promises = _2.map(files, async (value, name) => {
      const currPath = `${dir}/${name}`;
      if (typeof value === "string") {
...
var NemoFileManager = class {
  static getFilePath(rootDir, filename) {
    return path2.join(rootDir, NEMO_LOCAL_DICT_DIR, filename);
  }
  static loadJson(filePath) {
    if (fs2.existsSync(filePath)) {
      const obj = fs2.readJsonSync(filePath, { throws: false });
      return obj;
    }
    return null;
  }
  static writeJson(filePath, obj) {
    ...
    try {
      ...
      return fs2.writeFileSync(filePath, content);
    } catch (error2) {
  ...
  static loadFile(filePath) {
    if (fs2.existsSync(filePath)) {
      const fileContent = fs2.readFileSync(filePath, { encoding: "utf8" });
      return fileContent;
    }
    return null;
  }
  ...
  static appendFile(filePath, content) {
    if (fs2.existsSync(filePath)) {
      return fs2.appendFile(filePath, content);
    }
  }
  static removeFile(filePath) {
    if (fs2.existsSync(filePath)) {
      return fs2.remove(filePath);
    }
  }
};
...
var engines = [google, youdao, baidu];
var dict = {};
var dicPath;
...
function startCaseClassName(result) {
  let wordArray = _3.startCase(result).split(" ");
...
function getTemplate(rootPath, templateInfo) {
  ...
  const program2 = ts.createProgram([templateFileName], {
    outDir,
    rootDir: rootPath,
...
var Config = class extends DataSourceConfig {
  constructor(config) {
    super(config);
...
var getFileContent = (path10) => {
  return fs4.readFileSync(path10, { encoding: "utf-8" });
};
var pathIsExist = (path10) => path10 && fs4.existsSync(path10);
var OriginBaseReader = class {
  ...
  async translateChinese(jsonString) {
    ...
    try {
      ...
      chineseKeyCollect = _4.uniq(chineseKeyCollect.map((item) => item.includes("\xAB") ? item.split("\xAB")[0] : item));
  ...
  async fetchData() {
    ...
    try {
      if (judgeIsValidUrl(this.config.originUrl)) {
        ...
        const pathToSwagger = path5.resolve(process.cwd(), this.config.originUrl);
...
var Schema = class {
  static parseSwaggerSchema2StandardDataType(schema, defNames, classTemplateArgs = [], compileTemplateKeyword) {
    ...
    if (type === "array") {
      let itemsType = _5.get(items, "type", "");
      const itemsEnum = _5.get(items, "enum", []);
      const itemsRef = _5.get(items, "$ref", "");
      if (itemsType) {
...
var SwaggerInterface = class {
  ...
  static transformSwaggerV3Interface2Standard(inter, usingOperationId, samePath, defNames = []) {
    ...
    const responseSuccessContent = _5.get(inter, "responses.200.content", {});
    let responseSchema;
    if (responseSuccessContent) {
      const responseFormat = Object.keys(responseSuccessContent)[0];
      responseSchema = _5.get(responseSuccessContent, `${responseFormat}.schema`, {});
    }
    ...
    const standardInterface = new Interface({
      ...
      parameters: _5.unionBy(parameters, "name")
    });
  ...
  static transformSwaggerInterface2Standard(inter, usingOperationId, samePath, defNames = [], compileTemplateKeyword) {
    ...
    const responseSchema = _5.get(inter, "responses.200.schema", {});
    const response = Schema.parseSwaggerSchema2StandardDataType(responseSchema, defNames, [], compileTemplateKeyword);
    const parameters = (inter.parameters || []).map((param) => {
      let paramSchema;
...
function parseSwaggerV3Mods(swagger, defNames, usingOperationId) {
  ...
  _5.forEach(swagger.paths, (methodInters, path10) => {
    const pathItemObject = _5.cloneDeep(methodInters);
    ...
    _5.forEach(pathItemObject, (inter, method) => {
      inter.path = path10;
      inter.method = method;
      if (!inter.tags) {
  ...
  const mods = (swagger.tags || []).map((tag) => {
    ...
    if (hasChinese(tag.name)) {
      return new Mod({
        ...
        interfaces: _5.uniqBy(standardInterfaces, "name"),
...
function transformSwaggerData2Standard(swagger, usingOperationId = true, originName = "") {
  const draftClasses = _5.map(swagger.definitions, (def, defName) => {
  ...
  const baseClasses = draftClasses.map((clazz) => {
    ...
    const props = _5.map(properties, (prop, propName) => {
  ...
  return new StandardDataSource({
    baseClasses: _5.uniqBy(baseClasses, (base) => base.name),
...
function transformSwaggerV3Data2Standard(swagger, usingOperationId = true, originName = "") {
  const compileTemplateKeyword = "#/components/schemas/";
  const draftClasses = _5.map(swagger.components.schemas, (def, defName) => {
...
// src/reader/index.ts
var OriginType = /* @__PURE__ */ ((OriginType2) => {
  OriginType2["Hep"] = "Hep";
  OriginType2["SwaggerV3"] = "SwaggerV3";
  OriginType2["SwaggerV2"] = "SwaggerV2";
  OriginType2["RapV2"] = "RapV2";
  return OriginType2;
})(OriginType || {});
...
function analyWithName(preEntities, nextEntities) {
  return {
    newEntities: _7.differenceBy(nextEntities, preEntities, "name"),
    deletedEntities: _7.differenceBy(preEntities, nextEntities, "name"),
    modifiedEntities: _7.intersectionBy(preEntities, nextEntities, "name")
  };
}
function deepDifInterface(preInter, nextInter, preLabel) {
  ...
  updateParams.forEach((tParam) => {
    const preParam = _7.find(parameters, (param) => param.name === tParam.name);
    const nextParam = _7.find(nextInter.parameters, (param) => param.name === tParam.name);
    if (!_7.isEqual(preParam, nextParam)) {
      updateMsgs.push(`${preLabel}\u7684\u53C2\u6570 ${tParam.name} \u4FE1\u606F\u6709\u66F4\u65B0`);
    }
  });
  if (!_7.isEqual(preInter.response, nextInter.response)) {
...
function deepDifMod(preMod, nextMod) {
  ...
  updateInters.map((tInter) => {
    const preInter = _7.find(interfaces, (inter) => inter.name === tInter.name);
    const nextInter = _7.find(nextInterfaces, (inter) => inter.name === tInter.name);
...
function deepDifBo(preBo, nextBo) {
  ...
  updateProps.forEach((tProp) => {
    const preProps = _7.find(properties, (prop) => prop.name === tProp.name);
    const nextProps = _7.find(nextBo.properties, (prop) => prop.name === tProp.name);
    if (!_7.isEqual(preProps, nextProps)) {
...
function diff(preModels, nextModels, isMod = true) {
  ...
  modifiedEntities.forEach((tEntity) => {
    const preEntity = _7.find(preModels, (model) => model.name === tEntity.name);
    const nextEntity = _7.find(nextModels, (model) => model.name === tEntity.name);
    if (isMod) {
...
var Manager = class {
  ...
  existsLocal() {
    return fs5.existsSync(path6.join(this.currConfig.outDir, this.lockFilename)) || _8.some(
      this.allConfigs.map((config) => fs5.existsSync(path6.join(config.outDir, config.name ?? "", this.lockFilename)))
    );
  }
  async readLockFile() {
    try {
      let lockFile = path6.join(this.currConfig.outDir, this.lockFilename);
      const isExists = fs5.existsSync(lockFile);
      if (isExists) {
        const localDataStr = await fs5.readFile(lockFile, {
          encoding: "utf8"
        });
        if (this.allConfigs.length > 1 && this.currConfig.spiltApiLock) {
          this.regenerateFiles().then(() => fs5.rename(lockFile, `${lockFile}.bak`));
        }
        return JSON.parse(localDataStr);
      } else {
        const allFilePromises = this.allConfigs.map(async (config) => {
          const filePath = path6.join(config.outDir, config.name ?? "", this.lockFilename);
          const localDataStr = await fs5.readFile(filePath, {
            encoding: "utf8"
          });
          return JSON.parse(localDataStr);
        });
  ...
  dispatch(files) {
    return _8.mapValues(files, (value) => {
  ...
  setFilesManager() {
    ...
    const generators = this.allLocalDataSources.map((dataSource) => {
      ...
      if (_8.isFunction(generator.getDataSourceCallback)) {
...
function getMaxSamePath(paths, samePath = "") {
  ...
  if (paths.some((path10) => !path10.includes("/"))) {
    return samePath;
  }
  const segs = paths.map((path10) => {
    const [firstSeg, ...restSegs] = path10.split("/");
...
function getTemplatesDirFile(fileName, filePath = "templates/") {
  return fs6.readFileSync(__dirname.substring(0, __dirname.lastIndexOf("dist")) + filePath + fileName, "utf8");
}
function judgeTemplatesDirFileExists(fileName, filePath = "templates/") {
  return fs6.existsSync(__dirname.substring(0, __dirname.lastIndexOf("dist")) + filePath + fileName);
}
async function lookForFiles(dir, fileName) {
  const files = await fs6.readdir(dir);
  for (let file of files) {
    const currName = path7.join(dir, file);
    const info2 = await fs6.lstat(currName);
...
async function createManager(configFile = NEMO_CONFIG_FILE) {
  ...
  const manager = new Manager(PROJECT_ROOT, config, path7.dirname(configPath));
  await manager.ready();
  return manager;
}
...
var npmType = new Promise((resolve2) => {
  return resolve2("npm");
});
var yarnType = new Promise((resolve2) => {
  return resolve2("yarn");
});
var pkg = JSON.parse(fs6.readFileSync(path7.join(__dirname, "..", "package.json"), "utf8"));
...
var parseRoute = (route) => {
  ...
  const pathParams = _9.reduce(
    pathParamMatches,
    (pathParams2, match) => {
      const paramName = _9.replace(match, /\{|\}|\:/g, "");
      if (!paramName)
        return pathParams2;
      if (_9.includes(paramName, "-")) {
  ...
  const fixedRoute = _9.reduce(
    pathParams,
    (fixedRoute2, pathParam) => {
      return _9.replace(fixedRoute2, pathParam.$match, `\${${pathParam.name}}`);
    },
    route || ""
  );
  return {
...
var promptList = [
  ...
  },
  {
    ...
    message: "\u8BF7\u9009\u62E9\u6570\u636E\u6E90\u63A5\u53E3\u7C7B\u578B:",
    name: "originType",
    choices: Object.keys(OriginType).map((key) => OriginType[key]).filter((value) => typeof value === "string")
  },
  {
    type: "confirm",
  ...
  },
  {
    ...
    message: "\u8BF7\u9009\u62E9\u8BED\u8A00\u7C7B\u578B:",
    name: "languageType",
    choices: ["javaScript" /* javaScript */, "typeScript" /* typeScript */]
  }
];
async function generateNemoConfig() {
  const configPath = await lookForFiles(process.cwd(), NEMO_CONFIG_FILE);
  if (configPath) {
    const result = await inquirer.prompt({
      type: "confirm",
      name: "confirm",
      default: true,
      message: `\u68C0\u6D4B\u5230\u5DF2\u5B58\u5728 ${NEMO_CONFIG_FILE} \u6587\u4EF6\uFF0C\u7EE7\u7EED\u751F\u6210\u5C06\u8986\u76D6\u914D\u7F6E\u9879\uFF0C\u662F\u5426\u7EE7\u7EED\uFF1F`
    });
    if (!result.confirm) {
      return;
    }
  ...
  success("\u6587\u4EF6\u751F\u6210\u6210\u529F\u3002");
  info(`
    \u5176\u4F59\u914D\u7F6E\u9879\u8BF7\u53C2\u9605\u6587\u6863 https://gitlab.hundsun.com/whaleFE/nemo
  `);
}
function generateConfig(configPath, answers) {
  const { originUrl, templatePath, outDir, enableMocks, originType, languageType } = answers;
  const dirName = path8.join(process.cwd(), NEMO_CONFIG_FILE);
  let config = {};
  if (configPath) {
    try {
      ...
      throw new Error(`${NEMO_CONFIG_FILE} \u662F\u4E00\u4E2A\u8BED\u6CD5\u5F02\u5E38 JSON \u6587\u4EF6`);
    }
  }
  config.originUrl = originUrl.trim();
  ...
  fs7.writeFileSync(configPath || dirName, JSON.stringify(config, null, 2));
}

// src/action/check.ts
function assert2(expression, message) {
  if (!expression) {
    error(message);
    process.exit(1);
  }
...
async function checkApiLock() {
  info(`${NEMO_LOCK} \u6587\u4EF6\u68C0\u6D4B\u4E2D...`);
  const manager = await createManager();
  if (!manager) {
    info(`${NEMO_LOCK} \u6587\u4EF6\u68C0\u6D4B\u5931\u8D25\uFF0C\u8BF7\u786E\u8BA4\uFF01`);
    return;
  }
  ...
  try {
    ...
    localDatas.forEach((data) => {
      data.baseClasses.forEach((base) => {
        assert2(!!base.name, `\u63CF\u8FF0\u4E3A ${base.description} \u7684\u7C7B\u6CA1\u6709\u300Cname\u300D\u5C5E\u6027`);
        base.properties.forEach((prop) => {
          assert2(!!prop.name, `${base.name} \u7C7B\u7684\u67D0\u4E2A\u5C5E\u6027\u6CA1\u6709\u300Cname\u300D\u5C5E\u6027`);
        });
      });
      data.mods.forEach((mod) => {
        assert2(!!mod.name, `\u63CF\u8FF0\u4E3A ${mod.description} \u7684\u6A21\u5757\u6CA1\u6709\u300Cname\u300D\u5C5E\u6027`);
        mod.interfaces.forEach((inter) => {
          assert2(!!inter.name, `${mod.name} \u6A21\u5757\u7684\u67D0\u4E2A\u63A5\u53E3\u6CA1\u6709\u300Cname\u300D\u5C5E\u6027`);
          inter.parameters.forEach((param) => {
            assert2(!!param.name, `${mod.name} \u6A21\u5757\u7684 ${inter.name} \u63A5\u53E3\u7684\u67D0\u4E2A\u53C2\u6570\u6CA1\u6709 "name" \u5C5E\u6027`);
          });
        });
      });
    });
...
async function diffDataSource() {
  ...
  if (Array.isArray(modDiffs) && modDiffs.length) {
    console.log(`${chalk2.bgRedBright("\u6CE8\u610F\uFF1A\u6A21\u5757\u6709\u66F4\u65B0\uFF01!!")}`);
    console.log(`${chalk2.cyan("\u5177\u4F53\u4FE1\u606F\u5982\u4E0B\uFF1A")}`);
    console.log(modDiffs.map((mod) => `${chalk2.bold.yellow(mod.name)}(${mod.details.join("\n\r")})`).join("\n\r"));
  }
  if (Array.isArray(boDiffs) && boDiffs.length) {
    console.log("\n\r");
    console.log(`${chalk2.bgRedBright("\u6CE8\u610F\uFF1A\u57FA\u7C7B\u6709\u66F4\u65B0\uFF01!!")}`);
    console.log(`${chalk2.cyan("\u5177\u4F53\u4FE1\u606F\u5982\u4E0B\uFF1A")}`);
    console.log(boDiffs.map((bo) => `${chalk2.bold.magenta(bo.name)}(${bo.details.join("\n\r")})`).join("\n\r"));
  }
}
...
async function queryDataSource() {
  ...
  info(manager.allConfigs.map((conf) => conf.name).join("  "));
}

// src/action/select.ts
async function chooseDataSource(dsName) {
  const manager = await createManager();
  if (!manager) {
    info(`${NEMO_LOCK} \u6587\u4EF6\u68C0\u6D4B\u5931\u8D25\uFF0C\u8BF7\u786E\u8BA4\uFF01`);
    return;
  }
  await manager.selectDataSource(dsName);
}

// src/action/update.ts
async function updateBo(boName) {
  const manager = await createManager();
  if (!manager) {
    info(`${NEMO_LOCK} \u6587\u4EF6\u68C0\u6D4B\u5931\u8D25\uFF0C\u8BF7\u786E\u8BA4\uFF01`);
    return;
  ...
  await manager.regenerateFiles();
}

// src/action/updateMod.ts
...
// src/action/updateVersion.ts
var import_ora = __toESM(require_ora());
import { execSync as execSync2 } from "child_process";
var checkLatestVersion = async () => {
  const npm = await npmType;
  const latestVersion = execSync2(`${npm} view ${PKG_NAME} version`).toString("utf-8").trim();
...
var updateVersion_default = async (install = true) => {
  const checking = (0, import_ora.default)(`[${PKG_NAME}] \u6B63\u5728\u68C0\u67E5\u6700\u65B0\u7248\u672C...`);
  checking.start();
  try {
    const npm = await npmType;
    ...
    if (latestVersion && install) {
      const update = (0, import_ora.default)(`[${PKG_NAME}] \u5B58\u5728\u65B0\u7248\u672C\uFF0C\u5C06\u5347\u7EA7\u81F3 ${latestVersion}`);
      update.start();
      execSync2(`${npm} i -g ${PKG_NAME}`);
      update.stop();
...
function readDirRecursively(dirPath, callback) {
  fs8.readdirSync(dirPath).forEach((subPath) => {
...
async function scan() {
  ...
  const allRequests = new Set(
    allLocalDataSources.map((ds) => ds.mods).reduce((acc, cur) => [...acc, ...cur], []).map((mod) => mod.interfaces).reduce((acc, cur) => [...acc, ...cur], [])
  );
  ...
  currConfig.scannedRange.forEach((dir) => {
    readDirRecursively(dir, (currentPath) => {
      const fileContent = fs8.readFileSync(currentPath).toString();
  ...
  fs8.writeFileSync(
    outputFileName,
    JSON.stringify(
      [...unusedRequests].map(({ method, path: path10, description }) => ({ method, path: path10, description })),
      null,
      2
    )
  );
  info(`unused case percentage: ${unusedRequests.size} / ${allRequests.size}`);
  success(`done!
unused file is outputted to ${path9.resolve(rootPath, outputFileName)}`);
}

// src/action/genReport.ts
async function genReport() {
  ...
  await manager.openReport();
}

// src/cli.ts
var program = new Command();
program.version(PKG_VERSION, "-v, --version", "output the current version").name("nemo").usage("[\u547D\u4EE4] [\u914D\u7F6E\u9879]");
program.description("service interface api code generator via swagger schema.");
program.command("init").alias("i").description("nemo \u914D\u7F6E\u6587\u4EF6\u751F\u6210").action(generateNemoConfig);
program.command("check").alias("c").description(`\u68C0\u6D4B ${NEMO_LOCK} \u6587\u4EF6`).action(checkApiLock);
program.command("ls").alias("l").description("\u67E5\u770B\u6570\u636E\u6E90").action(queryDataSource);
program.command("diff").alias("d").description("\u5BF9\u6BD4\u6570\u636E\u6E90").action(diffDataSource);
program.command("select <dsName>").description("\u9009\u62E9\u6570\u636E\u6E90").alias("s").action((dsName) => {
  chooseDataSource(dsName);
});
program.command("updateBo <boName>").description("\u66F4\u65B0\u57FA\u7C7B").alias("ub").action((boName) => {
  updateBo(boName);
});
program.command("updateMod <modName>").alias("um").description("\u66F4\u65B0\u6A21\u5757").action((modName) => {
  updateMod(modName);
});
program.command("generate").alias("g").description("\u751F\u6210\u4EE3\u7801").action(genCode);
program.command("update").alias("u").description(`\u66F4\u65B0 ${PKG_NAME} \u81F3\u6700\u65B0\u7248\u672C`).action(() => updateVersion_default(true));
program.command("scan").alias("sn").description("\u626B\u63CF\u5E9F\u5F03\u63A5\u53E3").action(scan);
program.command("open").alias("o").description("\u751F\u6210\u63A5\u53E3\u53D8\u66F4\u8BB0\u5F55\u62A5\u8868").action(genReport);
program.parse(process.argv);
if (process.argv.length === 2) {
  program.outputHelp();
}
process.on("unhandledRejection", (error2) => {
  console.error("unhandledRejection", error2);
  process.exit(1);
});
