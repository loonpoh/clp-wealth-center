...
import * as _9 from "lodash";

// src/Core/Standard.ts
import * as _7 from "lodash";
...
var NemoFileManager = class {
  static getFilePath(rootDir, filename) {
    return path.join(rootDir, NEMO_LOCAL_DICT_DIR, filename);
  }
...
function startCaseClassName(result) {
  let wordArray = _2.startCase(result).split(" ");
...
var getFileContent = (path8) => {
  return fs2.readFileSync(path8, { encoding: "utf-8" });
};
var pathIsExist = (path8) => path8 && fs2.existsSync(path8);
var OriginBaseReader = class {
  ...
  async translateChinese(jsonString) {
    ...
    try {
      ...
      chineseKeyCollect = _3.uniq(chineseKeyCollect.map((item) => item.includes("\xAB") ? item.split("\xAB")[0] : item));
  ...
  async fetchData() {
    ...
    try {
      if (judgeIsValidUrl(this.config.originUrl)) {
        ...
        const pathToSwagger = path2.resolve(process.cwd(), this.config.originUrl);
...
var Schema = class {
  static parseSwaggerSchema2StandardDataType(schema, defNames, classTemplateArgs = [], compileTemplateKeyword) {
    ...
    if (type === "array") {
      let itemsType = _4.get(items, "type", "");
      const itemsEnum = _4.get(items, "enum", []);
      const itemsRef = _4.get(items, "$ref", "");
      if (itemsType) {
...
var SwaggerInterface = class {
  ...
  static transformSwaggerV3Interface2Standard(inter, usingOperationId, samePath, defNames = []) {
    ...
    const responseSuccessContent = _4.get(inter, "responses.200.content", {});
    let responseSchema;
    if (responseSuccessContent) {
      const responseFormat = Object.keys(responseSuccessContent)[0];
      responseSchema = _4.get(responseSuccessContent, `${responseFormat}.schema`, {});
    }
    ...
    const standardInterface = new Interface({
      ...
      parameters: _4.unionBy(parameters, "name")
    });
  ...
  static transformSwaggerInterface2Standard(inter, usingOperationId, samePath, defNames = [], compileTemplateKeyword) {
    ...
    const responseSchema = _4.get(inter, "responses.200.schema", {});
    const response = Schema.parseSwaggerSchema2StandardDataType(responseSchema, defNames, [], compileTemplateKeyword);
    const parameters = (inter.parameters || []).map((param) => {
      let paramSchema;
...
function parseSwaggerV3Mods(swagger, defNames, usingOperationId) {
  ...
  _4.forEach(swagger.paths, (methodInters, path8) => {
    const pathItemObject = _4.cloneDeep(methodInters);
    ...
    _4.forEach(pathItemObject, (inter, method) => {
      inter.path = path8;
      inter.method = method;
      if (!inter.tags) {
  ...
  const mods = (swagger.tags || []).map((tag) => {
    ...
    if (hasChinese(tag.name)) {
      return new Mod({
        ...
        interfaces: _4.uniqBy(standardInterfaces, "name"),
...
function transformSwaggerData2Standard(swagger, usingOperationId = true, originName = "") {
  const draftClasses = _4.map(swagger.definitions, (def, defName) => {
  ...
  const baseClasses = draftClasses.map((clazz) => {
    ...
    const props = _4.map(properties, (prop, propName) => {
  ...
  return new StandardDataSource({
    baseClasses: _4.uniqBy(baseClasses, (base) => base.name),
...
function transformSwaggerV3Data2Standard(swagger, usingOperationId = true, originName = "") {
  const compileTemplateKeyword = "#/components/schemas/";
  const draftClasses = _4.map(swagger.components.schemas, (def, defName) => {
...
function getTemplatesDirFile(fileName, filePath = "templates/") {
  return fs4.readFileSync(__dirname.substring(0, __dirname.lastIndexOf("dist")) + filePath + fileName, "utf8");
}
function judgeTemplatesDirFileExists(fileName, filePath = "templates/") {
  return fs4.existsSync(__dirname.substring(0, __dirname.lastIndexOf("dist")) + filePath + fileName);
}
async function lookForFiles(dir, fileName) {
  const files = await fs4.readdir(dir);
  for (let file of files) {
    const currName = path5.join(dir, file);
    const info2 = await fs4.lstat(currName);
...
async function createManager(configFile = NEMO_CONFIG_FILE) {
  ...
  const manager = new Manager(PROJECT_ROOT, config, path5.dirname(configPath));
  await manager.ready();
  return manager;
}
...
var pkg = JSON.parse(fs4.readFileSync(path5.join(__dirname, "..", "package.json"), "utf8"));
...
var parseRoute = (route) => {
  ...
  const pathParams = _6.reduce(
    pathParamMatches,
    (pathParams2, match) => {
      const paramName = _6.replace(match, /\{|\}|\:/g, "");
      if (!paramName)
        return pathParams2;
      if (_6.includes(paramName, "-")) {
  ...
  const fixedRoute = _6.reduce(
    pathParams,
    (fixedRoute2, pathParam) => {
      return _6.replace(fixedRoute2, pathParam.$match, `\${${pathParam.name}}`);
    },
    route || ""
  );
  return {
...
export {
  ...
  CodeGenerator,
  Config,
  DataSourceConfig2 as DataSourceConfig,
  FileStructures,
  FilesManager,
  Interface,
  LanguageType,
